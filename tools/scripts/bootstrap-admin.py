#!/usr/bin/env python3
"""Standalone bootstrap-admin script.

Creates a system `admin` user (and the tenant if missing) against an arbitrary
...
--------------
    ...
    BOOTSTRAP_ADMIN_EMAIL=admin@plaglens.local \\
    BOOTSTRAP_ADMIN_PASSWORD=changeme \\
    DATABASE_URL=postgresql+asyncpg://identity_app:pwd@localhost:5432/plaglens \\
        python tools/scripts/bootstrap-admin.py
...
the tenant is created if missing, an existing admin is left untouched,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Bootstrap a PlagLens system admin (idempotent)."
    )
    ...
    parser.add_argument("--tenant-slug",
                        default=os.getenv("BOOTSTRAP_ADMIN_TENANT_SLUG", "system"))
    ...
    os.environ["BOOTSTRAP_ADMIN_EMAIL"] = args.email
    os.environ["BOOTSTRAP_ADMIN_PASSWORD"] = args.password
    os.environ["BOOTSTRAP_ADMIN_TENANT_SLUG"] = args.tenant_slug

    _patch_pythonpath()

    # Import after patching PYTHONPATH so the identity-service package resolves.
    from identity_service.bootstrap_admin import main as _run  # type: ignore
