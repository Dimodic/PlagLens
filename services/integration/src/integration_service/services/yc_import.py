"""Yandex.Contest import-job orchestration (service layer).

This module owns the *import-JOB lifecycle* and the background workers that
drive a Yandex.Contest import end-to-end:

* job lifecycle on the ``ImportJob`` table — ``start_import_job`` /
  ``finalize_import_job`` / ``update_job_progress`` / ``stamp_autosync_last_run``
* the contest-import ``RemoteOperation`` Redis state (polled by the modal)
* the ``(course_id, contest_id) → homework_id`` idempotency mapping
* the downstream HTTP fan-out (course-service assignments, submission-service
  batchImport, identity bulk-import) wrapped by the worker functions
* the three background workers themselves: ``run_import_as_homework``,
  ``run_resync_submissions`` and ``run_sync_all_imported_contests``.

It was extracted verbatim from ``api/v1/yandex_contest.py`` (which had grown to
~2.6k LOC and exported these as router-private ``_``-helpers that
``services/scheduler.py`` and ``api/v1/sync.py`` reached into). The router now
imports the public names below; the behaviour is identical — this was a pure
move/rename.
"""
...
def normalize_language_hint(raw: str | None) -> str | None:
...
async def op_create(initial: dict[str, Any]) -> str:
...
async def op_update(op_id: str, **patch: Any) -> None:
...
async def op_get(op_id: str) -> dict[str, Any] | None:
...
async def record_imported_homework(
    *,
    config_id: str,
    contest_id: int,
    homework_id: str,
...
async def drop_imported_homework(
...
def _build_assignment_body(
    ...
    body: dict[str, Any] = {
        ...
        # (see ``import_submissions_for_aliases``). A wrong "cpp" guess
...
async def import_submissions_for_aliases(
    *,
    ...
    for alias, subs in by_alias.items():
        ...
        lang_votes: Counter[str] = Counter(
            hint
            for rs in subs
            if (hint := normalize_language_hint(getattr(rs, "language", None)))
        )
        if lang_votes:
...
async def run_import_as_homework(
    *,
    ...
    cached admin service token because the user's bearer is not
    ...
    try:
        ...
        async with httpx.AsyncClient(
            ...
            await op_update(op_id, homework_title=contest_name)

            # 2. Create homework with slug-retry.
            await op_update(op_id, stage="creating_homework")
            hw_slug_base = slugify(
            ...
            if homework_id is not None and cfg.id is not None:
                try:
                    await record_imported_homework(
                        config_id=str(cfg.id),
                        contest_id=contest_id,
            ...
            if alias_to_aid:
                ...
                sub_stats, sub_errors = await import_submissions_for_aliases(
                    adapter=adapter,
                    cfg=cfg,
                    contest_id=contest_id,
...
async def run_resync_submissions(
    *,
    op_id: str,
    cfg: Any,
    contest_id: int,
    ...
    try:
        ...
        if not token:
            ...
            await op_update(op_id, status="failed", stage=None, errors=[err])
            if job_id is not None:
                await finalize_import_job(
                    job_id=job_id,
                    config_id=str(cfg.id),
...
async def update_job_progress(job_id: str, **patch: Any) -> None:
...
async def stamp_autosync_last_run(config_id: str) -> None:
...
async def run_sync_all_imported_contests(
    *,
    job_id: str,
    cfg: Any,
    ...
    try:
        ...
        async with httpx.AsyncClient(
            ...
            for idx, (contest_id, hw_id_str, course_for_hw) in enumerate(
                ...
                # ``homework_exists`` already probes the GET endpoint,
