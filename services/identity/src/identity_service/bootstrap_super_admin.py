"""Bootstrap a system super-admin if missing.
...
Behaviour
---------
* Reads ``BOOTSTRAP_SUPER_ADMIN_EMAIL``, ``BOOTSTRAP_SUPER_ADMIN_PASSWORD`` and
  optional ``BOOTSTRAP_SUPER_ADMIN_TENANT_SLUG`` (default ``system``) from the
  environment.
...
logger = logging.getLogger("identity_service.bootstrap_super_admin")
logging.basicConfig(level=os.getenv("LOG_LEVEL", "INFO"))


async def _bootstrap() -> int:
    email = os.getenv("BOOTSTRAP_SUPER_ADMIN_EMAIL", "").strip().lower()
    password = os.getenv("BOOTSTRAP_SUPER_ADMIN_PASSWORD", "")
    tenant_slug = os.getenv("BOOTSTRAP_SUPER_ADMIN_TENANT_SLUG", "system").strip().lower()

    if not email or not password:
        logger.info(
            "BOOTSTRAP_SUPER_ADMIN_EMAIL or BOOTSTRAP_SUPER_ADMIN_PASSWORD "
    ...
    async with factory() as session:
        ...
        if existing is not None:
            logger.info(
                "super_admin already exists for tenant=%s (user_id=%s) — skipping.",
                tenant_slug,
                existing.id,
            )
        ...
        await session.commit()
        logger.info(
            "Bootstrapped super_admin user=%s tenant=%s id=%s",
            email,
            tenant_slug,
            user.id,
        )
...
def main() -> int:
    try:
        ...
        logger.exception("bootstrap_super_admin failed: %s", exc)
        return 1
